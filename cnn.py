...
class PerceptNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.features = nn.Sequential(
            ...
            conv3,
            normalisation_4)
        self.linear = nn.Linear(128, 5) #NEW
        self.softmax = nn.Softmax(dim=-1) #NEW
    ...
    def forward(self,
        ...
        # output = torch.mean(output,dim=(1,2,3)) # OLD
        output = torch.mean(output,dim=(2,3)) #NEW
        output = self.linear(output) #NEW
        output = self.softmax(output) #NEW
        return output
...
def cnn_preds():
    ...
    device = 'cpu'
    model = PerceptNet(dims=1)
    model.load_state_dict(torch.load('models/cnnce.pth'))
    model.eval()

    with torch.no_grad():
        preds = torch.zeros((975))
        for batch, (X, y) in enumerate(test_dataloader):
            # preds[batch] = model(X)
            preds[batch] = torch.argmax(model(X), dim=-1)
    return preds
